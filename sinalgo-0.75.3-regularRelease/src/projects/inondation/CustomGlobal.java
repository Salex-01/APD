...
import projects.inondation.nodes.nodeImplementations.EndNode;
import projects.inondation.nodes.nodeImplementations.FloodNode;
...
*/ 
	...
	private void buildGrid(int length, int width) {		
		...
		// nodes
		sinalgo.tools.Tools.removeAllNodes();
		FloodNode [][] theNodes = new FloodNode[length][width];
	...
	private void buildTree(int degree, int numStages) {
		sinalgo.tools.Tools.removeAllNodes();
		ArrayList<FloodNode> nodes = new ArrayList<FloodNode>();

		// the root
		double posX = sinalgo.configuration.Configuration.dimX / 2;
		double posY = 50;
		FloodNode node = new FloodNode();
		node.setPosition(posX, posY, 0);
		node.finishInitializationWithDefaultModels(true);
		nodes.add(node);
	...
	private void buildRing(int numOfNodes) {
		...
		// nodes
		FloodNode [] theNodes = new FloodNode[numOfNodes];
